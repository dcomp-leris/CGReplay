...
# Receiver-side output normalization:
# Keep incoming stream as-is for decoding/QR detection, then optionally resize for display/storage.
receiver_output_cfg = config["gamer"].get("receiver_output_resolution", {})
receiver_fixed_output = bool(config["gamer"].get("fixed_output_resolution", False))
receiver_output_width = int(receiver_output_cfg.get("width", config["encoding"]["resolution"]["width"]))
receiver_output_height = int(receiver_output_cfg.get("height", config["encoding"]["resolution"]["height"]))
receiver_output_resolution = (receiver_output_width, receiver_output_height)
...
with open(frame_log, "w") as f:
    f.write("frame_id,frame_counter,fps,retry_status,recv_resolution,output_resolution\n")
...
def normalize_output_frame(frame):
    """Resize frame to a fixed receiver output size when enabled."""
    if frame is None:
        return frame
    if not receiver_fixed_output:
        return frame
    if (frame.shape[1], frame.shape[0]) == receiver_output_resolution:
        return frame
    return cv2.resize(frame, receiver_output_resolution, interpolation=cv2.INTER_LINEAR)
...
while True:
    ...
    recv_resolution = (frame.shape[1], frame.shape[0])
    output_frame = normalize_output_frame(frame)
    output_resolution = (output_frame.shape[1], output_frame.shape[0])
    ...
    with lock:
        latest_frame = output_frame.copy()  # Update frame for live display
    ...
    if frame_id:
        ...
        if frame_id!=frame_counter and (previous_frame_id+1)!=frame_id and frame_id!=1:
            ...
            log_frame_buffer.append(f"{frame_id},{frame_counter},{current_fps},{1},{recv_resolution},{output_resolution}\n")
            if len(log_frame_buffer) >= buffer_size: open(frame_log, "a").writelines(log_frame_buffer); log_frame_buffer.clear()
            ...
            log_frame_buffer.append(f"{frame_id},{frame_counter},{current_fps},{0},{recv_resolution},{output_resolution}\n")
            if len(log_frame_buffer) >= buffer_size: open(frame_log, "a").writelines(log_frame_buffer); log_frame_buffer.clear()
        ...
        # Write logs if buffer is full 
        # FID, FPS, Retry Status [noremal:0, retry:1, No_QR:2]
        log_frame_buffer.append(f"{frame_id},{frame_counter},{current_fps},{2},{recv_resolution},{output_resolution}\n")
        if len(log_frame_buffer) >= buffer_size: open(frame_log, "a").writelines(log_frame_buffer); log_frame_buffer.clear()
        #pass
        #frame_counter = frame_counter + 1
