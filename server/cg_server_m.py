...
source_folder_path = os.path.join(REPO_ROOT, "Sources", game_name)
folder_path = os.path.join(SCRIPT_DIR, game_name)            # Prepared local server frames in png format
...
encoding_resolution = (resolution_width, resolution_height)
...
def prepare_local_server_frames(source_dir, dest_dir, target_resolution, max_frame_id):
    """Copy frames from Sources/<game> to server/<game> after resizing to encoding resolution."""
    if not os.path.isdir(source_dir):
        raise FileNotFoundError(f"Source frame directory does not exist: {source_dir}")

    os.makedirs(dest_dir, exist_ok=True)

    # Clean old prepared PNG files first so each run is deterministic.
    ...
    for name in os.listdir(source_dir):
        ...
        if frame_id <= max_frame_id:
            frame_candidates.append((frame_id, name))

    frame_candidates.sort(key=lambda item: item[0])
    if not frame_candidates:
        raise RuntimeError(
            f"No numeric PNG frames <= stop_frm_number ({max_frame_id}) found in: {source_dir}"
        )

    for frame_id, src_name in frame_candidates:
        src_path = os.path.join(source_dir, src_name)
        frame = cv2.imread(src_path)
        if frame is None:
            continue
        resized = cv2.resize(frame, target_resolution, interpolation=cv2.INTER_AREA)
        dst_path = os.path.join(dest_dir, f"{frame_id:04d}.png")
        cv2.imwrite(dst_path, resized)

    print(
        f"Prepared {len(frame_candidates)} frames: {source_dir} -> {dest_dir} at resolution {target_resolution}"
    )
...
def stream_frames(game_name):
    ...
    while idx < len(png_files):
        ...
        # Frames are pre-resized during local preparation.


        qr_data = (
            f"Frame ID: {frame_id}, rcv_timestamp: {timestamp}, resolution: {encoding_resolution}, bitrate:{bitrate}"
        )
        ...
        with open(frame_log, "a") as f:
            f.write(
                f"{frame_id},{encoding_resolution},{len(frame_byte)},{GOP},{current_srv_fps},{processing_time},{bitrate}\n"
            )
...
if __name__ == "__main__":
    ...
    prepare_local_server_frames(source_folder_path, folder_path, encoding_resolution, stop_frm_number)
    print(f'Started streaming to {cg_server_port} from local folder {folder_path} ... ')
